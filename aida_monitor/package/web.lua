...
<div class="font-bay"><div class="font-head"><div><strong>TTF FONT MATCHER</strong><small>上传电脑端同一字体；布局命中字体族后优先使用，否则自动回退内置中文字体。</small></div><span class="flag" id="fontFlag">CHECKING</span></div><div class="font-grid"><div><label for="fontFile">TTF 文件 / 最大 4 MB</label><input class="file" id="fontFile" type="file" accept=".ttf,font/ttf,application/x-font-ttf"></div><div><label for="fontFamily">AIDA64 字体名称</label><input id="fontFamily" placeholder="Noto Sans SC"></div></div><div class="row" style="margin-top:10px"><div><label for="subpixel">子像素排列</label><select id="subpixel"><option value="rgb">RGB（推荐）</option><option value="bgr">BGR</option><option value="off">关闭</option></select></div><div><label>内置回退字体</label><input value="AIDA Noto Sans SC" disabled></div></div><div class="upload-actions"><button class="primary" type="button" id="uploadFont">上传并匹配 TTF</button><button type="button" id="resetFont">恢复内置字体</button><a class="button" href="/apps/aida_monitor/font/aida_noto_sans_sc.ttf" download>下载内置 TTF</a></div><progress id="fontProgress" max="100" value="0"></progress><p class="micro" id="fontHint">选择字体后会自动读取字体族名称；如识别不准，请按 AIDA64 字体下拉框中的名称修改。</p><code class="font-state" id="fontState">FONT ENGINE // CHECKING</code></div>
...
function runtime(d){$("rtStatus").textContent=d.status||"--";$("rtPage").textContent=(d.page||0)+"/"+(d.pages||0)+(d.page_source==="tilt"?"·TILT":"");$("rtItems").textContent=d.items||0;let img=$("rtImages"),skip=d.images_skipped||0;img.textContent=(d.images_loaded||0)+"/"+((d.images_loaded||0)+skip);img.className=skip?"warn":"";let uploaded=d.font_source==="uploaded"&&d.font_match;$("fontFlag").textContent=uploaded?"MATCHED":"FALLBACK";$("fontFlag").className="flag "+(uploaded?"match":"");let request=d.font_requested_families||"none reported";$("fontState").textContent="FONT // "+(d.font||"AIDA Noto Sans SC")+" · "+(d.font_source||"default").toUpperCase()+" · "+(d.font_selection||"bundled default")+" · REQUEST ["+request+"] · "+bytes(d.font_bytes);$("fontState").className="font-state "+(uploaded?"":"warn")}
function apply(d){state=d;$("host").value=d.host||"";$("port").value=d.port||9999;$("layout").value=d.layout_path||"/";$("stream").value=d.path||"/sse";$("subpixel").value=d.font_subpixel||"rgb";$("tiltCooldown").value=d.tilt_page_cooldown_ms||1000;if(d.font_custom_family)$("fontFamily").value=d.font_custom_family;runtime(d);sync()}
...
$("resetFont").onclick=async()=>{try{tone("正在恢复内置字体...");let q=new URLSearchParams({font_default:"1",host:$("host").value.trim(),port:$("port").value.trim()}),r=await fetch(API+"/save?"+q,{cache:"no-store"}),d=await r.json();if(!r.ok||!d.ok)throw Error(d.error||"恢复失败");$("fontProgress").value=0;$("fontFamily").value="";apply(d);tone("已恢复内置 AIDA Noto Sans SC。","ok")}catch(e){tone(e.message,"err")}};
...
function Web.new(opts)
  ...
  function self:snapshot(ok, message)
    ...
    return {
      ...
      font = runtime.font or tostring(self.config.vector_font_family or "AIDA Noto Sans SC"),
